...
(function ($) {
    ...
    let _addItem = function(widgetOptions, e, $elem) {
        ...
        if (relatedFormWrapper) {
            var index = $(relatedFormWrapper).find('.df-widget-item').last().data('index');
            if (index === undefined) {
                index = -1;
            }
            index = index + 1;
            ...
            _createItem($elem, widgetOptions, count);
        }
    ...
    let _makeSortable = function () {
        $('.df-sortable').sortable({
            handle: ".df-sortable-handle",
            items: ".df-widget-item",
            stop: function (event, ui) {
                var _sorter = $(this);
                _sorter.find('.df-widget-item').each(function (k, v) {
                    var _row = $(v);
                    var textarea = $(_row).find('textarea');
                    textarea.each(function (j, input) {

                        if ($(input).prev().hasClass('mce-tinymce')) {
                            //reinit every tinyMCE instances
    ...
    let _createItem = function ($elem, widgetOptions, count) {
        jQuery.ajax({
            'cache': false,
            'type': 'POST',
            'dataType': 'json',
            'data': {
                className: $elem[0].getAttribute("data-classname"),
                container: widgetOptions.widgetContainer,
                index: count
            },
            ...
            'beforeSend': function () {
                if (count === widgetOptions.limit) {
                    alert(widgetOptions.maxMsg);
                    return false;
                }
            },
            'complete': function () {},
            'url': '/dynamic-form/dynamic-form/new-row'
        });
    };

    let _restoreSpecialJs = function(widgetOptions) {
        _makeSortable();
    };
