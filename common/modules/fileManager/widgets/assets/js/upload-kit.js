(function ($) {
    jQuery.fn.yiiUploadKit = function (options) {
        ...
        var methods = {
            ...
            createItem: function (file) {
                ...
                var wrapbox = 'popup-wrap';
                if (options.multiple) {
                    name += '[' + index + ']';
                    wrapbox += '-' + index;
                    id += '-' + index;
                }
                var titleValue = file.img_title;
                var altValue = file.img_alt;
                if (file.img_title === undefined || file.img_title === null) {
                    titleValue = '';
                }
                if (file.img_alt === undefined || file.img_alt === null) {
                    altValue = '';
                }
                var wrapper = $('<div>', {'class': wrapbox + ' wrapbox', 'style': 'display:none'})
                    .append('<span class="btn wrap-trigger wrap-close" data-item="' + wrapbox + '">X</span>')
                    .append('<br>')

                    .append('<label for="' +  id + '-img_title' + '"/>' + options.titleLabel + '</label>')
                    .append('<input name="' + name + '[img_title]' + '"' +
                        'value="' + titleValue + '"' +
                        'id="' + id + '-img_title" ' +
                        'class="wrapbox-field"' +
                        '/>')
                    .append('<br>')
                    .append('<label for="' +  id + '-img_alt' + '"/>' + options.altLabel + '</label>')
                    .append('<input name="' + name + '[img_alt]' + '"' +
                        'value="' + altValue + '"' +
                        'id="' + id + '-img_alt" ' +
                        'class="wrapbox-field"' +
                        '/>');
                ...
                var item = $('<li>', {"class": "upload-kit-item done"})
                    ...
                    .append($('<span/>', {
                        "class": options.extraFields ? "glyphicon glyphicon-pencil wrap-trigger file-meta align-left" : "hidden",
                        "data-url": file.delete_url,
                        'data-item': wrapbox
                    }))
                    .append($('<span/>', {
                        "class": options.extraFields ? "glyphicon glyphicon-remove-circle remove align-right" : "glyphicon glyphicon-remove-circle remove",
                        "data-url": file.delete_url
                    }))
                    .append(wrapper);
    ...
    $(document).on('click', '.wrap-trigger', function (e) {
        var rel = $(this).attr('data-item');
        $('.' + rel).slideToggle(5);
    })
})(jQuery);
