...
class DynamicForm extends \yii\base\Widget
{
    ...
    const VIEW_FILE = '@common/modules/dynamicForm/views/dynamic-form/_form';
    const FIELD_VIEW_FILE = '@common/modules/dynamicForm/views/dynamic-form/_form_fields';
    ...
    protected function initOptions()
    {
        ...
        $this->_options['minMsg']          = Yii::t('app', 'Min amount of items: {min}', ['min' => $this->min]);
        $this->_options['maxMsg']          = Yii::t('app', 'Max {max} items have been reached', ['max' => $this->limit]);
    ...
    private function removeItems($content)
    {
        ...
        $crawler->rewind();
        /** @var \DOMNode $currentCrawler */
        $currentCrawler = $crawler->current();
        $root->appendChild($document->importNode($currentCrawler, true));
        $domxpath = new \DOMXPath($document);
        $converter = new CssSelectorConverter();
        $crawlerInverse = $domxpath->query($converter->toXPath($this->widgetItem));
     ...
     * @return array
     */
    public static function generateForms(int $count, string $relatedClass, $postParams = []): array
    {
        $relatedModels = [];
        if (!empty($postParams)) {
            $formName = (new $relatedClass())->formName();
            $count = count($postParams[$formName]);
        }
        for ($i = 0; $i < $count; $i++) {
            /** @var ActiveRecord $relatedModel */
            $relatedModel = new $relatedClass();
            $relatedModel->relModelIndex = $i;
            $relatedModel->loadDefaultValues();
            $relatedModels[$i] = $relatedModel;
        }
